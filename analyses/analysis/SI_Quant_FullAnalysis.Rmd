...
##Data management
The data is coded in with one row per participant. Need to convert that two multiple rows per participant with one trial per row. Additionally, need to perform some sanity checks to catch any errors in data coding. 
```{r}
...
To test prediction of Alternatives Hypothesis
```{r}
...
Compute means and 95% CIs, graph
```{r}
library(RColorBrewer)
my_palette = c(brewer.pal(4, "YlGnBu")[c(2,3,4,5)])
ms_exp1 <- data_exp1 %>%
  mutate(correct = as.numeric(correct))%>%
  group_by(condition, type, trial_type, agegroup)%>%
  multi_boot_standard("correct", na.rm = TRUE)

pdf(file="./exp1_performance.pdf", width=7, height=4)
...
First, aggregate a dataframe with which to run t-tests
```{r}
...
Testing performance for "Some" and "None" against ceiling performance on "All" across age groups
```{r}
#t-tests against "all"
#4-4.5
#all-some
...
T-tests: Scalar vs. ad-hoc - is there significantly lower performance for critical implicature trials in Scalar condition
```{r}
...
Bimodal performance for trial types indicate that participants respond consistently either correctly or incorrectly
```{r}
dip_exp1 <- data_exp1 %>%
...
###Full correlations
Testing whether low performance on "some" and "none" trials is correlated
```{r}
#full correlations
...
#testing for random effect of item within trial type
...
##Read in data
Data format conversion same as above, with same checks.
```{r}
...
##Classify trials as first or second half
Testing prediction of Alt. Hypothesis.
```{r}
...
Testing performance for "some" and "none" against performance on "none" across age groups within Experiment 2.
```{r}
...
Testing whether performance is significantly improved after removing adhoc trials from Exp. 1.
```{r}
exp1_ms <- data_exp1 %>%
  filter(condition == "Scalar")%>%
  mutate(correct = as.numeric(correct))
...
Replicating bimodal performance for "some" and "none".
```{r}
...
##Incorrect selection
What kinds of errors are children making?
```{r}
...
#does performance change by half of experiment?
...
#testing with no random effect of trial type
...
Data format conversion checks same as above.
```{r}
...
Do we see any changes from previous data? Testing against Experiment 2
```{r}
#first create a df 
...
Testing performance on "some" and "none" against "all" across age groups.
```{r}


d <- aggregate(correct ~ type + agegroup +sub_ID, data=data_si, mean)
...
##Correlations (full)
Replicating correlated performance for "some" and "none"
```{r}
...
#cor with age
cor.test(dip_exp3$Some, dip_exp3$age)
cor.test(dip_exp3$None, dip_exp3$age)
```
##Correlation (partial, controlling for age)
Because we found that DCCS (inhibitory control measure) was significantly correlated with age, we control for age in correlations between all three measures (SI, GQ, and DCCS). 

Here, testing whether correlation exists between "some" and "none" when controlling for age.
```{r}
library(ppcor)
...
Data format conversion and checks same as above. 

Coding for GQ: "Correct" responses outlined in Barner, Chow, & Yang (2009).
None - correct if number given == 0
Some - correct if 2 <= number given <= 7
Most - correct if 5 <= number given <= 7
All - correct if number given == 8
```{r}
...
How many items (on average) do children give in response to a prompt across age groups?
```{r}
hist_data <- data_gq %>%
  filter(prompt != "Most")%>%
...
#Experiment 3 - DCCS
Inhibitory control measure - Dimension Change Card Sort. Data format conversion same as above.
```{r}
...
##Full analyses
Running correlations between SI, GQ, and DCCS. First, create long-form dataframe for group analyses. This is done by aggregating mean correct by SID by trial type and age group:
- trial type = all, some, none (SI)
- trial type = all, some, none (GQ)
- trial type = pre/post switch (DCCS)

After each individual df is compiled, all around bound together by SID
```{r}
#si df
...
##T-tests
###DCCS
Is performance significantly different than chance?
```{r}
