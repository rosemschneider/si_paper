...
exp4 <- data1 %>%
  dplyr::select(SID, age, agegroup, sex, Exclude, prompt, selection, selection_type, correct, correct_factor)
...
num_kids <- exp4 %>%
  distinct(SID, age, agegroup, sex)%>%
  group_by(agegroup)%>%
  dplyr::summarise(n = n(), mean = mean(age), sd = sd(age), median = median(age), 
            n_f = sum(sex == "F"), 
            n_m = sum(sex == "M"))
