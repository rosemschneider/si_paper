...
We ran a logistic mixed effects model, predicting a correct response as an interaction of age, condition (ad-hoc or scalar) and trial type (implicature or control), with random effects of participant and trial type.\footnote{All mixed effects models were fit in R using the lme4 package. The model specifications are as follows:  \small{\tt{correct ~ trial_type * condition * age + (trial_type | Subject_ID}}.} We found that performance was slightly lower for scalar trials than ad-hoc trials ($\beta$ = -8.02, \textit{p} = .09), and that there was a significant interaction between condition and trial type, such that performance was significantly worse on scalar implicature trials ($\beta$ = 16.07, \textit{p} = .0187). We also found a significant 3-way interaction between condition, trial type, and age, such that performance on scalar implicature trials decreased with age ($\beta$ = --4.07, \textit{p} = .0079). There were no significant effects of adding trial order (trials in the first half vs. second half of the experiment), indicating that performance did not change throughout the course of the experiment.
