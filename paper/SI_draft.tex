...
In (Table \ref{tab:lit_review}), we present a brief review of developmental implicature research. This is a non-exhaustive list, but does capture the wide range of paradigms and dependent variables across studies. In this literature, we found also that most studies targeted relatively narrow age bins (12--18 months, although several are wider, see cf. \citeNP{papafragou2004}). In reviewing previous research, we observed differing syntactic structure in scalar implicature prompts, which may influence whether an implicature is made. For example, recent work indicates that the use of the partitive is a particularly strong cue for scalar implicatures \cite{degen2014}. In sum, the diversity of measures across these different studies may result in children's inconsistent performance, as many things in context affect the strength of scalar implicatures \cite{degen2015}. However, research on children's ability to compute scalar implicatures suggests that their fragile performance might have less to do with their general pragmatic knowledge per se, and more to do with their knowledge of particular scales. 
...
\begin{figure} 
 \begin{center} 
  ...
  \caption{\label{fig:exp3_DCCS} Proportion of correct responses by each age group in post-switch trials. Error bars show 95\% confidence intervals computed by non-parametric bootstrap.} 
 \end{center} 
\end{figure}
