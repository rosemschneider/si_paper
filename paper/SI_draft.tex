...
\abstract{
%Using language requires speakers and listeners to make pragmatic inferences that extend beyond the literal sense of an utterance. 
What is the source of children's difficulties with pragmatic implicatures? For example, adult listeners quickly and easily infer from the statement ``\textit{Some} of the cookies are oatmeal raisin'' that the others might not be. Children, however, struggle with these types of statements and often fail to make scalar implicatures using quantifiers. Children's performance varies tremendously across studies and tasks, however, which makes direct comparisons between datasets difficult. To address this issue, we designed a novel experimental paradigm, which both minimized task demands for participants, and enabled comparisons across different tasks. In Experiment 1, we explored children's computation of both ad-hoc (contextual) and scalar (quantifier) implicatures, and found that while older 4-year-olds succeeded with ad-hoc descriptions, they still performed poorly with quantifier descriptions. We also found bimodal and correlated performance with the quantifiers ``some'' and ``none''. In Experiment 2, we attempted to isolate children's sources of difficulty with these terms by including only scalar trials, and found that while performance increased, children continued to struggle with the quantifiers ``some'' and ``none.'' In Experiment 3, we explored possible sources of developmental difficulty in this task. We found that children who had difficulty with ``some'' and ``none'' in our implicature task made similar errors in a quantifier knowledge task. Taken together, our results provide more detailed data on the development of pragmatic implicatures, and suggest that difficulty with scalar implicatures may in fact be rooted in a lack of quantifier knowledge. 
~\\
...
%For example, an adult who hears ``I ate \emph{some} of the cookies'' would likely expect that the speaker did not eat \emph{all} the cookies. Similarly, that adult would also infer from the utterance ``I ate the sugar cookies,'' that the speaker ate \emph{only} the sugar cookies, and not the other varieties.
...
%Across these studies however, children's difficulty with scalar implicatures suggests that their fragile performance might have less to do with general pragmatic knowledge per se, and more to do with their knowledge of particular scales. 
...
In Experiment 3, we explored the role of inhibitory control and lack of quantifier knowledge in children's failures with the quantifiers ``some'' and ``none''. In this individual differences study, we included an inhibitory control (the Dimensional Change Card Sort \cite{zelazo2006}) and a quantifier knowledge measure (the Give-Quantifier task \cite{barner2009}) in addition the our scalar implicature task. Overall, our findings suggest that while preschoolers' computation of scalar implicatures can be supported by stronger recognition of the lexical alternatives, their failure in making these implicatures may be rooted in difficulty comprehending and contrasting relevant quantifiers.
...
The correlated responses for``some'' and ``none'' trials in both Experiments 1 and 2 presented a particularly interesting puzzle, and suggested that children's difficulty with these quantifiers could have a common root, either in quantifier knowledge or inhibitory control.  ``None'' is not typically considered part of the same Horn scale as ``some'' and ``all'' (because ``all'' entails ``some,'' but ``some'' does not entail ``none''---and in fact entails the opposite), but it is nonetheless a lexical contrast along the same quantifier scale. In comprehension of the quantifier ``some,'' listeners must make several pragmatic decisions about the appropriateness of that quantifier given the target set-size, as well as the knowledge of the competing alternatives. Recent work has suggested that when accounting for both of these considerations in processing ``some,'' the quantifier ``none'' is a particularly salient alternative \cite{franke2014}. This finding supports the strong correlation we observed between children's related performance on these two quantifiers: If children's quantifier scale is not fully-established, they may fail to reason about a very salient competing alternative when computing a scalar term, and perform similarly across both ``some'' and ``none.'' 

One possibility is that children's knowledge of the whole quantifier scale plays a role in scalar implicature, though knowledge of logically-false alternatives is not involved in the computations outlined by most theoretical accounts \cite<e.g.>{barner2011}. Another is that performance on ``none'' and ``some'' trials may be correlated because both scalar implicature and negation comprehension might require inhibiting another response---the positive alternative in the negative case, and the stronger alternative in the implicature case. We explored these two alternatives in Experiment 3, and found support for the possibility that quantifier knowledge, rather than inhibitory control, drives this correlation between children's responses with the quantifiers ``some'' and ``none.''

%Quantifiers are a difficult lexical class to acquire for children, as their corresponding quantities can be a) determined by the target set-size, and b) not mutually exclusive. For example, in a set-size of 8 (such as in the Give-Quantifier task), the quantifier ``some'' can be used to correctly describe anywhere from 2 to 7 objects \cite{barner2009}, although adult-like responses tend to center around the mean of the set-size \cite{franke2014}.  The difficulty these changing set-sizes entail can be seen in 3--4-year-olds' asymmetrical success in assigning two objects (out of four) to the number prompt ``two,'' and failure in assigning all four objects to the quantifier prompt ``some'' \cite{hurewitz2009}. Our results are consistent with previous research on children's failure on quantifier-knowledge tasks, and suggest that scalar implicature comprehension relies on the ability to assess relevant alternatives on an established quantifier scale.
